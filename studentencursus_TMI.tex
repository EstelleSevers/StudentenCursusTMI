...
\begin{document}
	...
	Een speciaal geval kan twee punten zijn die dezelfde y-waarde hebben, dan kan de event queue het niet juist sorteren. Dit kan opgelost worden door de doorlooplijn zogezegd wat schuiner te laten lopen. Een ander probleem is het samenvallen van events (het einde van een lijnstuk is gelijk aan het begin van een ander lijnstuk.) Hier moeten dan tijdens het implementeren bepaalde keuzes gemaakt worden, maar dit kan ook opgelost worden. %TODO moet hier nog iets bij? 
	...
	\begin{description}
		\item[Constructie]:\\
		Deze structuur is vooral om een interval in 2d te zoeken, er wordt dan afwisselend tussen x- en y-as afgewisseld om de ruimte verder op te splitsen. De pseudocode om zo'n bomen op te stellen is hieronder te vinden: 
		\begin{figure}[H]
			\centering
	...
	\begin{itemize}
		...
		\item Er kan een grid worden geplaatst over de veelhoek, afhankelijk van in welk vierkantje het punt ligt weten we al op voorhand of het in de veelhoek zit of niet. In de vierkantjes kan ook een zijde van de veelhoek liggen, dan moeten er meer testen volgen. 
	\end{itemize}
	...
	\begin{description}
		...
		\item[Incrementeel algoritme]: Telkens een nieuwe site toevoegen aan het voronoi diagramma, er wordt dan met de cirkeleigenschappen bekeken welke voronoi-edges opnieuw berekend moeten worden. 
	...
	\begin{description}
		\item[Status golffront]: Gesorteerde sequentie van paraboolsegmenten: Breekpunten knopen, sites bladeren. 
	...
	\end{description}
	De constructie van het voronoi diagramma zal dus bestaan uit een golffront dat uitgebreid wordt als een site bereikt wordt en ingekrompen wordt als een cirkel-event plaatsvindt. 
	...
	\begin{itemize}
		...
		\item De edges van de convex omhullende zijn steeds deel van de triangulatie.
	\end{itemize}
	...
	Deze veeltermcurve zal gegeven n+1 punten een curve maken voor de punten p met als parameterwaarde u (gaande van 0 tot n). De curve zal dus afhankelijk zijn van alle punten en een kleine wijziging zal heel de curve aanpassen. Dit is in het algemeen nadelig aangezien de curves plaatselijk aanpassen wel een handige tool is. De curve volgt ook de punten niet volledig, het is heel uiteenlopend. Het is duidelijk dat dit niet echt fantastisch is. 
	...
	\begin{itemize}
		\item De curve is een gewogen som van Lagrangeveeltermen met 'vector'-coëfficiënten $\rightarrow$ n-de graads, continu, ...
	...
	Het voordeel is dat de curve zal interpoleren door de gegeven punten maar anderzijds zal de curve oscilleren tussen interpolatiepunten en dit effect zal ook erger worden bij meer punten. Zoals hierboven gezegd is het curveverloop ook heel gevoelig aan verplaatsingen, wat ook niet wenselijk is. 
	...
	Als we de graad blijven vehogen, krijgen we controlepunten die convergeren naar de curven en die dus de curve beter en beter beginnen voorstellen. De curve zelf wijzigt niet! Graadverhoging is dus een goede manier om zo'n curve grafisch voor te stellen. De nieuwe controlepunten zullen een combinatie zijn an de buren van die nieuwe controlepunten, bij het toevoegen van zo een punt worden dus enkel 'hoekjes' van de veelhoek afgesneden. 
	...
	Zoals je het al hoort uit de naam, is het hier de bedoeling twee curves samen te stellen aan de hand van de controlepunten. De curve kan er vreemd uitzien, hiervoor kunnen we de continuiteit wat aanpassen zodat de curve er zelf eleganter uitziet. Hier zijn ook wat stellingen bij: 
	\begin{figure}[H]
		\centering
	...
	\\
	\\
	Deze keer zal men voor elke vertex een doorlooplijn maken die rondgaat en als event de andere vertices heeft. Indien er een vertex verborgen is voor p, dan is er een edge die de lijn snijdt. De events zijn dan de vertices van S en de status ids dan een binaire boom met alle edges die de doorlooplijn snijdt. Hieronder de pseudocode om $G_{vis}$ op te stellen. 
	\begin{figure}[H]
		\centering
